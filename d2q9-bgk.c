...
int accelerate_flow(const t_param params, t_speed* cells, int* obstacles)
{
  ...
  #pragma omp parallel for schedule(static) shared(cells,obstacles)
...
float fusion(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles){
  ...
  #pragma omp parallel for schedule(static) shared(cells,tmp_cells, obstacles) reduction(+:tot_u,tot_cells)
