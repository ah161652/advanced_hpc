...
kernel void fusion1(global t_speed* cells,
                            ...
                            int nx,
                            int ny,
                            local float* local_u_1 ,
                            global float* partial_u_1,
                            local float* local_u_2,
                            global float* partial_u_2,
                            float omega, float accel, float density)
{
  ...
  ** we don't send a negative density */
  ...
  local_u_1[cell_index] = 0.0f;


  // calculate cell local_u value
    local_u_1[cell_index] = sqrt((u_x * u_x) + (u_y * u_y));
    ...
    ** we don't send a negative density */
        ...
        && jj==0)
    {
      ...
      //init local_u for this cell to 0
      local_u_2[cell_index] = 0.0f;


      // calculate cell local_u value
        local_u_2[cell_index] = sqrt((u_x * u_x) + (u_y * u_y));
        ...
        if (local_id_x == 0 && local_id_y == 0) {
          ...
          //init to 0
          work_group_total_u = 0.0f;
          partial_u_2[group_index] = 0;


          //sum all cells in work group
          for (size_t i=0; i<total_work_items; i++) {
              work_group_total_u += local_u_2[i];

          }


          //add work group sums to global arrays
          partial_u_2[group_index] = work_group_total_u;
